...
def random_crop(img, boxes):
    ...
    for attempt in range(10):
        ...
        c=b-a+1
        object_area=c[:,0]*c[:,1]
        object_min_area=object_area.min().tolist()
        expand_p=1
        if object_min_area<40*40:
            # expand_p=random.uniform((object_min_area/(64*64)),(object_min_area/(128*128)))
            expand_p=(object_min_area/(40*40))
            target_area =  expand_p* area
            aspect_ratio = random.uniform(3. / 4, 4. / 3)
        else:
            target_area = random.uniform(0.56, 1.0) * area
            aspect_ratio = random.uniform(3. / 4, 4. / 3)
    ...
    x1=(torch.cat((boxes[:,0],torch.Tensor([x])))).min()
    y1=(torch.cat((boxes[:,1],torch.Tensor([y])))).min()
    x2=(torch.cat((boxes[:,2],torch.Tensor([x+w])))).max()
    y2=(torch.cat((boxes[:,3],torch.Tensor([y+h])))).max()

    x1=(x+(x1-x)+(x2-(x+w)))
    y1=(y+(y1-y)+(y2-(y+h)))

    (x1, y1, x2, y2)= torch.stack((x1, y1, x1+w, y1+h)).tolist()
